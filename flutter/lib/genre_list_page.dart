...
class GenreListPage extends StatefulWidget {
  ...
  GenreListPage({required this.refreshGenresList});
 ...
 void _fetchGenres() async {
  try {
    ...
    switch (response.statusCode) {
      case 200:
        setState(() {
          _genres = (json.decode(response.body) as List)
  ...
  List<Widget> _buildGenreList(List<Genre> genres) {
    if (genres.isEmpty) {
      ...
      return genres.map((genre) => ListTile(
                          ...
                          Navigator.push(
                            ...
                            MaterialPageRoute(
                              builder: (context) => DeleteGenrePage(
                                genre: _selectedGenre!,
                                refreshGenreList: refreshGenresList,
                              ),
                            ),
                          ).then((_) {
                            refreshGenresList();
